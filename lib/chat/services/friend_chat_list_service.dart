// lib/chat/services/friend_chat_list_service.dart - 트립프렌즈 앱(프렌즈용)
...
class ChatListService {
  ...
  ChatListService({required this.friendsId}) {
  ...
  Future<void> _loadTranslations() async {
    ...
    _noMessageText = _translationService.get('no_message', '메시지 없음');
  ...
  Future<List<ChatListItem>> getChatList() async {
    try {
      print('채팅 목록 가져오기 시작 - 사용자 ID: $friendsId');

      // 사용자의 채팅 목록 경로
      final snapshot = await _database.ref()
          .child('users/$friendsId/chats')
          .get();

      if (!snapshot.exists || snapshot.value == null) {
        print('채팅 목록 없음: users/$friendsId/chats');
        return [];
      }
      ...
      // 병렬로 모든 고객 정보 가져오기
      final customerInfoFutures = <String, Future<Map<String, dynamic>>>{};

      for (var chatId in chatDataMap.keys) {
        final otherUserId = chatDataMap[chatId]!['otherUserId'] as String? ?? '';
        if (otherUserId.isNotEmpty) {
          customerInfoFutures[otherUserId] = _chatService.getCustomerInfo(otherUserId);
        }
      }

      // 모든 고객 정보 병렬로 가져오기 기다림
      final customerInfoResults = <String, Map<String, dynamic>>{};
      for (var entry in customerInfoFutures.entries) {
        try {
          customerInfoResults[entry.key] = await entry.value;
        } catch (e) {
          print('고객 정보 로드 오류 (건너뜀): ${entry.key} - $e');
          // 오류 발생 시 기본값 설정
          customerInfoResults[entry.key] = {
            'name': '고객',
      ...
      for (var chatId in chatDataMap.keys) {
        ...
        // 이미 가져온 고객 정보 사용
        if (otherUserId.isNotEmpty && customerInfoResults.containsKey(otherUserId)) {
          final customerInfo = customerInfoResults[otherUserId]!;
          final customerName = customerInfo['name'] ?? '고객';
          final customerImage = customerInfo['profileImageUrl'];
  ...
  Future<void> deleteChat(String chatId) async {
    try {
      // 사용자의 채팅 목록에서 채팅방 삭제
      await _database.ref()
          .child('users/$friendsId/chats/$chatId')
          .remove();
